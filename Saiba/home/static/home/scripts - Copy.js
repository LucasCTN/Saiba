...
function appendCommentSection(commentSectionId, getCommentApiEndpoint, page) {
    $.ajaxSetup({
        beforeSend: function (xhr, settings) {
            if (!csrfSafeMethod(settings.type) && !this.crossDomain) {
                var csrftoken = getCookie("csrftoken");
    ...
    $.ajax({
        ...
        data: $(this).serialize(),
        success: function (data) {
            var comment_page = getCommentPage(data);

            for (i = 0; i < comment_page.comments.length; i++) {
                var comment = comment_page.comments[i];
                $("#" + commentSectionId).append(comment);
            }

            return comment_page.next_page;
        }
    })
}

function getCommentPage(json_comments) {
    var comment_page = {};
    comment_page.comments = [];
    comment_page.next_page = json_comments.next;
    ...
    for (i = 0; i < json_comments.results.length; i++) {
        ...
        var comment_replies = comment.find('#replies');
        ...
        comment_page.comments.push(comment);
    }
...
function createCommentPage(commentSectionId, json_comments) {
    for (i = 0; i < json_comments.results.length; i++) {
        var comment = createComment(json_comments.results[i].id, json_comments.results[i].author,
                                    json_comments.results[i].update_date, json_comments.results[i].content,
                                    json_comments.results[i].author_username, json_comments.results[i].author_slug,
        ...
        for (j = 0; j < json_comments.results[i].replies.length; j++) {
            ...
            var commentPage = $('#comment-' + commentId + ' #replies');
            commentPage.append(reply);
